...
tic()
kospi_map_df <- kospi_df %>% 
  mutate(data = map_df(`종목코드`, ~get_company_info(.x)))
toc()

kospi_map_df %>% 
  write_rds("data/kospi_map_df.rds")

```

## `for` 루프 버젼 {#financial-statement-kospi-for}


```{r kospi-company-info-for}
## 회사코드를 넣으면 기업정보를 반환시키는 함수
library(tictoc)


for(i in 1:nrow(kospi_df)) {
  cat(i, ": ", kospi_df$회사명[i])
}
